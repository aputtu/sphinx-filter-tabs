...
"""
    ...
    assert label.text.strip() == xss_payload, "Tab name should match payload textually but be escaped in HTML"
...
"""
    ...
    assert legend.text.strip() == xss_payload, "Legend text should match payload textually but be escaped in HTML"
...
"""
    ...
    assert 'body { background: url' not in theme_content, "CSS injection payload found in generated theme CSS!"
